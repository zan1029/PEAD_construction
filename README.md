...
**项目**：LLM Signal × 财报公告 CAR — baseline 部分（CAR / SUE / Controls，暂不含 Moderator 与 LLM signal）
...
| 满窗 CAR（DRIFT 完整 60 天）· C2C | 1996-01-02 | **2026-04-01** | 需要公告后 61 个交易日，C2C 数据到 2026-06-30 |
| 满窗 CAR（DRIFT 完整 60 天）· O2O | 1996-01-02 | **2025-12-29** | O2O（yifei v5）数据止于 2026-03-30 |
...
**末端的两个不同截点**：`flag_short_window` 标记 DRIFT 不足 60 天的事件。
若分析要求满窗，C2C 用到 2026-04-01、O2O 用到 2025-12-29；不要求满窗则可用到 2026-05-14。
...
| **EVOL** | Earnings Volatility | 盈余波动率 | 过去 16 季**季节差分** EPS 的标准差 |
| **EPERSIST** | Earnings Persistence | 盈余持续性 | 同一序列的一阶自相关 |
...
**EVOL / EPERSIST 的构造**：
1. `eps_adj = epspxq / ajexq`（折到同一拆股基准），要求 `ajexq ≥ 0.01`（见 §6.3）
2. 季节差分 `Δ4 = eps_adj_t − eps_adj_{t−4季}`，用 `fyearq×4 + fqtr` 严格对齐季度，不靠行序
3. 滚动 16 季（最少 4 个观测）：标准差 → **EVOL**；一阶自相关 → **EPERSIST**
4. 自相关用滚动和向量化（`rolling.apply` 慢 10 倍：5 分钟 → 31 秒）
...
| 部分 | 内容 | 产出 |
|---|---|---|
| A | 组合排序（BT 1989 方法）：十分位表、事件时间路径、对冲组合、实时分位稳健性 | `build/car_path.parquet`, `build/pead_paths.png` |
| B | 截面回归 (0a) `SUE + controls`，两窗口 × 两口径 | `build/baseline_results.csv` |
| C | 截面回归 (0b) 加 `X × SUE` 交互项 | 同上 |
| D | 稳健性：连续 SUE / 2000 年后 / 剔除微盘股 | — |
