...
### 已完成：LLM signal 回归（`回归结果补充.ipynb` §2.3 / §3.3 / §5）

LLM signal = 窗口内各日 $(1+\overline{pred})$ 连乘减 1，回归前标准化为 1 个标准差为单位。
两个窗口：$[d,\ d+1]$（2 个交易日）与 $[d-1,\ d+1]$（3 个交易日）。
`pred` 训练于 open-to-open 收益，**O2O 列是同源口径**，C2C 并列报出。
样本为窗口内有新闻的事件，约 14 万。

| 节 | 设定 | 核心结果 |
|---|---|---|
| §5.2 (1) | LLM 主效应 | **DRIFT O2O +0.0027\*\*\* / +0.0023\*\*\***。漂移窗口与训练目标不重叠，是干净证据：LLM signal 每高 1 个标准差，公告后 60 日多出约 0.25 个百分点 |
| §5.2 (2) | 加 `SUE × LLM` | **DRIFT 四格全显著为正**（O2O 最高 +0.0098\*\*\*）。盈余意外与新闻同向时漂移更大，两条信息渠道相互放大 |
| §5.4 (3a) | \|CAR drift\| ~ 新闻条数 | **四格全 1% 显著**（$t$ 5.8–6.8）。新闻数翻倍对应漂移绝对幅度多约 0.62 个百分点。**注意** $\|CAR\|$ 会机械捡到收益波动率，回归未控住这一项 |
| §5.4 (3b) | `SUE × log(1+NNEWS)` | ANN 上 $t$ 12–15（新闻越多、当期反应越强）；DRIFT 上为正但弱 |
| §3.3 | `ATT × LLM`（$\beta_5$） | DRIFT 四格符号一致为负，与 distraction 假说方向相符；只有 $[d,d+1]$ 达常规显著性（C2C 5%、O2O 10%） |
| §2.3 | `ADOPT × LLM`（$\beta_5$） | 两个窗口在 ANN 上符号相反（+0.0015\*\* vs −0.0016\*\*），DRIFT 四格全不显著。**结论不成立** |

方程里 $\{SUE,\ M,\ LLM\}$ 的两两交互全部纳入（$SUE\times M$、$SUE\times LLM$、$M\times LLM$），
控制变量同时与 $SUE$ 和 $LLM$ 交互。

**ANN 窗口的机械成分**：`pred` 的训练目标是 open$_t\to$open$_{t+1}$ 的收益，
而 $CAR^{ANN}$（O2O）覆盖同一段区间。预测是滚动样本外的，但被解释变量与训练目标重合，
...
- **更好的 AI 采用度量**：连续的采用强度替代 2022-11-30 的 0/1 断点 —— §2.2 与 §2.3 都显示
  纯日历断点识别不出来
- **更多 LLM 窗口**：$[d-5,\ d-1]$、$[d+2,\ d+61]$ 等，改 `build_llm_signal.py` 顶部一行即可
